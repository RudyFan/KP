...
def get_top_pair_loss(n):
    def top_pair_loss(scores, targets, debug=False):
        ...
        top_true, top_true_arg = torch.log(true_pairs).max(dim=1)  # max(log(p)), p=sigmoid(s)
        if debug:
        ...
        if false_ants is not None: # We have no false antecedents when there are no pairs
...
def get_ranking_loss(n):
    def ranking_loss(scores, targets):
        ...
        tmp_loss = scores.add(1).add(top_true.unsqueeze(1).neg()) # 1 + scores - top_true
...
def run_model(args):
    print("Training for", args.all_pairs_epoch,
          args.top_pairs_epoch, args.ranking_epoch, "epochs")
    # Tensorboard server 可视化工具
    ...
    # Construct model
    print("🏝 Build model")
    ##h1：number of hidden unit on layer 1
    ## SIZE_PAIR_IN = 2 * SIZE_MENTION_EMBEDDING + SIZE_FP # Input to the mentions pair neural network
    ##SIZE_SINGLE_IN = SIZE_MENTION_EMBEDDING + SIZE_FS  # Input to the single mention neural network
    ...
    if args.weights is not None:
        ...
        ##Start from a previously saved checkpoint file
    ...
    eval_evaluator = ConllEvaluator(model, eval_dataset, args.eval, args.evalkey,
                               embed_path, args)
    ...
    print("Dataloader parameters: batchsize", args.batchsize, "numworkers", args.numworkers)
    ##batchsize=600
    ##Number of workers for loading batches
    batch_sampler = NCBatchSampler(dataset.mentions_pair_length, shuffle=True,
                                   batchsize=args.batchsize)
    dataloader = DataLoader(dataset, collate_fn=padder_collate, batch_sampler=batch_sampler,
                            num_workers=args.numworkers, pin_memory=args.cuda)
    ...
    def run_epochs(start_epoch, end_epoch, loss_func, optim_func, save_name, lr, g_step, debug=None):
        ...
        for epoch in range(start_epoch, end_epoch):
            ...
            epoch_loss = 0
            for batch_i, (m_idx, n_pairs_l, batch) in enumerate(zip(mentions_idx, n_pairs, dataloader)):
                ...
                scores = model(inputs)
                if debug is not None and (debug == -1 or debug in m_idx):
                    print("Scores:\n" + "\n".join("|".join(str(s) for s in s_l) for s_l in scores.data.cpu().numpy()))
            ...
            print('|| min/epoch {:5.2f} | est. remaining time (h) {:5.2f} | loss {:.2e}'.format(elapsed_epoch/60,
                elapsed_all/3600*float(end_epoch-epoch)/float(epoch-start_epoch+1), epoch_loss))
            ...
            if epoch % args.conll_train_interval == 0:
                ...
                elapsed = time.time() - start_time
                print('|| min/train evaluation {:5.2f} | F1_conll {:5.2f}'.format(elapsed/60, f1_conll))
    ...
    if args.startstage is None or args.startstage == "allpairs":
        ##all_pairs_lr  all pairs pre-training learning rate
        ##  all pairs pre-training l2 regularization
...
if __name__ == '__main__':
    ...
    parser.add_argument('--numworkers', type=int, default=8, help='Number of workers for loading batches')
    parser.add_argument('--startstage', type=str, help='Start from a specific stage ("allpairs", "toppairs", "ranking")')
    parser.add_argument('--startstep', type=int, help='Start from a specific step')
    parser.add_argument('--checkpoint_file', type=str, help='Start from a previously saved checkpoint file')
    parser.add_argument('--log_interval', type=int, default=10, help='test every X mini-batches')
    parser.add_argument('--conll_eval_interval', type=int, default=10, help='evaluate eval F1 conll every X epochs')
    parser.add_argument('--conll_train_interval', type=int, default=20, help='evaluate train F1 conll every X epochs')
